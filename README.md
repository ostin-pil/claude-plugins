...
The GitHub repo is `ostin-pil/ProseMint` (private, provisional name); the tool, CLI, package, and plugin are `prose-lint` and may be renamed once a final name is chosen. Done: the standalone engine and byte-for-byte regression gate, the per-project config layer, the Claude Code plugin (skill, command, marketplace), the MCP server, and the reusable CI action. A drift guard fails the suite if the upstream Untype scanner gains a category prose-lint has not ported. Remaining: the Bounce migration to consume prose-lint (a fallback-chain shim, staged for a Bounce session) and the v2 mechanized banlist. See CHANGELOG.md for the phase log.
