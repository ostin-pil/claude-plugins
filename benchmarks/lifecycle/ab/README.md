...
Run 2 (isolated control, Haiku/Sonnet/Opus arms, N=3) is in `RESULTS.md`. The short version: the isolation matters, since the same `ambiguous-finalize` control swings from 3/3 confounded on Opus to 1/3 isolated, and the uplift is scenario-dependent. `ambiguous-finalize` shows tier-independent uplift (the control never reliably refuses, even on Opus, where it sees the two candidates and rationalizes a tiebreak anyway; the skill refuses 3/3 wherever it can execute). `branch-birth` shows tier-shaped uplift (Opus re-derives it from the manifest's `integration_ref` and ties 3/3, Sonnet needs the skill 0/3 vs 3/3, Haiku cannot execute the multi-step skill at all). The full analysis, including why the skill arm's failures stay conservative, is in `RESULTS.md`.
