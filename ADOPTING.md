...
> 2. Inspect the repo to infer the **build command(s)**, the **test command(s)**, and a **glob that marks a code change** (e.g. `*.ts`, `*.go`, `*.py`, `*.rs`). Read whatever manifest exists: `package.json`, `Cargo.toml`, `go.mod`, `pyproject.toml`, `Package.swift`, `Makefile`, `build.gradle`. Not every project has a real build step or a real test step; when there is none, set that field to `none` rather than inventing one. Most Python libraries have no build step, and an editable or dependency install (`pip install -e .`) counts as setup rather than a build. One exception to declining a test step: a repo whose code is shell scripts with no test framework should use `shellcheck` over those scripts (for example `shellcheck scripts/*.sh`) as its test command, the conventional check for shell. If ambiguous, ask me before guessing.
