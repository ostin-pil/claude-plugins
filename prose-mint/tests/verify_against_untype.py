...
Runs the live Untype bin/check-prose-bulk.sh and the extracted
prose-lint over the same CI-relevant doc set in the *current* Untype tree
...
Manual check (needs the live Untype checkout), like regen_golden.py:
    python3 tests/verify_against_untype.py
"""
...
def main() -> int:
    if not OLD.exists():
        print(f"no live Untype scanner at {OLD}", file=sys.stderr)
        return 2
