...
4. **Target: remote `$BRANCH` gone.** If `git -C "$MAIN" ls-remote --heads <remote> "$BRANCH"` is non-empty (gh's `--delete-branch` aborted before deleting it, finding 3): `gh api -X DELETE "repos/{owner}/{repo}/git/refs/heads/$BRANCH"` (REST, no `git push`, no GraphQL).
